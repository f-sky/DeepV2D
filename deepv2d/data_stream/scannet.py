...
class ScanNet:
    def __init__(self, dataset_path, n_frames=4, r=6, one_pair_per_scene=True, test_split='deepv2d'):
        self.dataset_path = dataset_path
        self.n_frames = n_frames
        self.height = 480
        self.width = 640
        self.one_pair_per_scene = one_pair_per_scene
        ...
        if self.test_split == 'deepv2d':
            ...
            for i in range(0, len(test_frames), 4):
                ...
                if self.one_pair_per_scene:
                    ...
                    if scan not in scan_visited:
                        scan_visited.append(scan)
                        num_img_per_scan = len(os.listdir(osp.join(self.dataset_path, scan, 'color')))
                        for imageid_1 in range(0, num_img_per_scan, 10):
                            imageid_2 = imageid_1 + 10
                            if imageid_2 < num_img_per_scan:
                                test_data.append((scan, imageid_1, imageid_2))
        elif self.test_split == 'official':
            test_scans = np.loadtxt('/data/scannet/scannet/splits/scannetv2_test.txt', dtype=np.unicode_)
            for scanid in test_scans:
                num_img_per_scan = len(os.listdir(osp.join(self.dataset_path, scanid, 'color')))
                for imageid_1 in range(0, num_img_per_scan, 10):
                    imageid_2 = imageid_1 + 10
                    if imageid_2 < num_img_per_scan:
                        test_data.append((scanid, imageid_1, imageid_2))
        return test_data

    def test_set_blob_iterator(self, index):
    ...
    def test_set_iterator(self, index):
        scanid, imageid_1, imageid_2 = self.test_data[index]
        ...
        for i in [0, dt, -3 * s, -2 * s, -s, s, 2 * s, 3 * s]:
            ...
            image = cv2.resize(image, (640, 480))
            images.append(image)
            pose = np.loadtxt(os.path.join(scandir, 'pose', '%d.txt' % otherid), delimiter=' ')
