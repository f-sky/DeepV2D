...
def bnrelu(x, self=None):
    x = slim.batch_norm(x)
    if self is not None and not hasattr(self, 'sbn'):
        self.sbn = x
    return tf.nn.relu(x)
...
def conv2d(x, dim, stride=1, bn=True, self=None):
    if bn:
        return slim.conv2d(bnrelu(x, self=self), dim, [3, 3], stride=stride)
    else:
        raise NotImplementedError()
        return slim.conv2d(tf.nn.relu(x), dim, [3, 3], stride=stride)


def res_conv2d(x, dim, stride=1, self=None):
    if stride == 1:
        y = conv2d(conv2d(x, dim, self=self), dim)
        # if self is not None and not hasattr(self, 'net1y'):
        #     self.net1y = y
    else:
        y1 = conv2d(x, dim)
        # if self is not None and not hasattr(self, 'ys21'):
        #     self.ys21 = y1
        y = conv2d(y1, dim, stride=2)
        # if self is not None and not hasattr(self, 'ys2'):
        #     self.ys2 = y
