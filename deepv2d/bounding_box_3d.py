...
class Box3DList(object):
    ...
    def __init__(self, bbox_3d, mode="xyzhwl_ry", ry=None, pose=None, frame="rect"):
    ...
    def __getitem__(self, item):
        bbox_3d = Box3DList(self.bbox_3d[item], self.mode, pose=self.pose)
        return bbox_3d
    ...
    def convert(self, mode):
        ...
        if mode == "corners":
            ...
            box_3d_list = Box3DList(bbox_3d, mode=mode)
    ...
    def transpose(self, method):
        ...
        bbox_3d = torch.cat(corners, dim=-1)
        box_3d_list = Box3DList(bbox_3d, mode="corners")

        return box_3d_list

    # Tensor-like methods
    def to(self, device):
        bbox_3d = Box3DList(self.bbox_3d.to(device), self.mode)
        return bbox_3d
    ...
    def enlarge_box3d(self, extra_width):
        ...
        box_3d_list = Box3DList(large_boxes3d, mode="xyzhwl_ry").convert(self.mode)
        return box_3d_list
    ...
    @staticmethod
    def from_disprcnn_box3dlist(box3dlist):
        # from disprcnn.structures.bounding_box_3d import Box3DList as B3DL
        return Box3DList(box3dlist.bbox_3d, box3dlist.mode)

    def filter_point_cloud(self, pointcloud):
        """
        filter pointcloud
        @param pointcloud: nx3
        @return: bool tensor with shape (len(self), n)
        """
        from monotrack.utils.utils_3d import filter_bbox_3d
        assert pointcloud.shape[1] == 3
        if not isinstance(pointcloud, torch.Tensor):
            pointcloud = torch.tensor(pointcloud).float()
        all_corners = self.convert('corners').bbox_3d.reshape(-1, 8, 3)
        keeps = []
        for corners in all_corners:
            _, _, keep = filter_bbox_3d(corners, pointcloud)
            keeps.append(keep)
