...
def make_predictions(args):
    ...
    with tf.Session(config=tfconfig) as sess:
        ...
        predictions = []
        for i in trange(697):
            # for i, data_blob in enumerate(db.test_set_iterator()):
            ...
            # pickle.dump([keyframe_image, keyframe_depth, intrinsics],
            #             open(osp.join(args.output_dir, str(i) + '.pkl'), 'wb'))
...
def evaluate(groundtruth, predictions, min_depth=1e-3, max_depth=80):
    ...
    for (t_id, depth_gt) in groundtruth:
        ...
        if len(depth_pr) == 0 and len(depth_gt) == 0:
            depth_metrics = {k: 0 for k, v in depth_metrics.items()}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='kitti_image_depth_preds')
    parser.add_argument('--input_pose', default=False, action='store_true')
