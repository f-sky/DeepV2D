...
def make_predictions(args):
    ...
    with tf.Session(config=tfconfig) as sess:
        ...
        db = ScanNet(args.dataset_dir, one_pair_per_scene=not args.recons,
                     test_split=args.test_split)
        bar = trange(len(db.test_data))
        for i in bar:
            if i < len(db.test_data) * args.start:
                continue
            test_data_blob = db.test_set_blob_iterator(i)
            ...
            if not args.ignore_cache and osp.exists(depth_cache_path) and osp.exists(pose_cache_path):
                ...
                test_blob = db.test_set_iterator(i)
                images, intrinsics = test_blob['images'], test_blob['intrinsics']
                camposes = test_blob['poses'] if args.input_campose else None
                iters = 1 if args.input_campose else 5
                # timer.tic()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='scannet_output', type=str)
